...
export const UserSidebarItem: FC<Props> = ({ item }) => {
  ...
  return (
    <UserSidebarItemStyle onClick={() => navigate(item.pathname)} active={isActive()}>
      <Icon size={ICON_SIZE} strokeWidth={STROKE_WIDTH} />
      {item.id === "friends" && friendRequests.length > 0 && <IconBadge>{friendRequests.length > 9 ? "10+" : friendRequests.length}</IconBadge>}
