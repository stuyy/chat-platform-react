...
export const ConversationSidebarItem: React.FC<Props> = ({ conversation }) => {
  ...
  return (
    <>
      <ConversationSidebarItemStyle onClick={() => navigate(`/conversations/${conversation.id}`)} selected={parseInt(id!) === conversation.id}>
        <img src={hasProfilePicture() ? CDN_URL.BASE.concat(recipient?.profile?.avatar!) : defaultAvatar} alt="avatar" className={styles.conversationAvatar} />
        <div className={styles.contentContainer}>
          <span className={styles.conversationName}>{`${recipient?.firstName} ${recipient?.lastName}`}</span>
