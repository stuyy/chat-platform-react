...
export const MessagePanelHeader = () => {
  ...
  const displayName =
    user?.id === conversation?.creator.id ? `${conversation?.recipient.firstName} ${conversation?.recipient.lastName}` : `${conversation?.creator.firstName} ${conversation?.creator.lastName}`
  const groupName = group?.title || "Group"
  const headerTitle = type === "group" ? groupName : displayName
  return (
    <>
      {showModal && <AddGroupRecipientModal showModal={showModal} setShowModal={setShowModal} />}
      <MessagePanelHeaderStyle>
        <div>
          <span>{headerTitle}</span>
        </div>
        <GroupHeaderIcons>
          {type === "group" && user?.id === group?.owner?.id && <PersonAdd cursor="pointer" size={30} onClick={() => setShowModal(true)} />}
          {type === "group" && <PeopleGroup cursor="pointer" size={30} onClick={() => dispatch(toggleSidebar())} />}
        </GroupHeaderIcons>
      </MessagePanelHeaderStyle>
    </>
  )
