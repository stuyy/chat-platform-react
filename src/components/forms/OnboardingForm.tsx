...
export const OnboardingForm = () => {
  ...
  return (
    <form className={styles.onboardingForm} onSubmit={onSubmit}>
      ...
      <OnboardingAboutField id="about" maxLength={200} value={about} onChange={(e) => setAbout(e.target.value)} />
      {source && (
        <UploadedAvatarContainer>
          <div className="side">
            <UploadedAvatar src={source} alt="avatar" />
            <div className="fileName">asdjhasduiashdjisadgsayhudgaudasdjhasduiashdjisadgsayhudgaud</div>
            <FiFileMinus size={40} color="#ff0000" onClick={reset} />
          </div>
        </UploadedAvatarContainer>
      )}
      <UploadAvatarButton onClick={handleClick} ref={imageContainerRef}>
        <label ref={labelRef} htmlFor="file" onClick={(e) => e.preventDefault()}>
          Upload Avatar
        </label>
        <FileInput id="file" type="file" accept="image/jpg, image/jpeg, image/png" ref={fileInputRef} onChange={onFileChange} />
      </UploadAvatarButton>
