...
export const FriendContextMenu = () => {
  ...
  const getUserFriendInstance = () => (user?.id === selectedFriendContextMenu?.sender.id ? selectedFriendContextMenu?.receiver : selectedFriendContextMenu?.sender)

  const removeFriend = () => {
    if (!selectedFriendContextMenu) return
    dispatch(toggleContextMenu(false))
    dispatch(removeFriendThunk(selectedFriendContextMenu.id)).then(() => socket.emit("getOnlineFriends"))
  }
